"""
多资产交易代理（使用 LogisticAgentWithLLMGate）
对多个股票分别使用独立的 LogisticAgentWithLLMGate 获取信号
...
from trader.agent.agent_logistic_with_llm_gate import LogisticAgentWithLLMGate
...
class MultiAssetLogisticAgentWithLLMGate(TradingAgent):
    """
    多资产交易代理（使用 LogisticAgentWithLLMGate）
    对每支股票使用独立的 LogisticAgentWithLLMGate 获取信号
    然后使用 LLM uncertainty gate 进行风险控制，最后进行权重归一化
    """
    ...
    def __init__(
        ...
        name: str = "MultiAssetLogisticAgentWithLLMGate",
        ...
        # 为每支股票创建独立的 LogisticAgentWithLLMGate
        self.agents: Dict[str, LogisticAgentWithLLMGate] = {}
        for stock_code in stock_codes:
            self.agents[stock_code] = LogisticAgentWithLLMGate(
        ...
        logger.info(
            f"创建 MultiAssetLogisticAgentWithLLMGate: {len(stock_codes)} 支股票, "
            f"每支股票使用独立的 LogisticAgentWithLLMGate{parallel_info}"
        )
    
    def score(self, stock_code: str, engine: BacktestEngine) -> float:
        """
        计算股票的 score（使用该股票对应的 LogisticAgentWithLLMGate）
    ...
    def get_all_weights(
        ...
        apply_llm_gate: bool = True
    ) -> Dict[str, float]:
        """
        获取所有股票的权重（归一化后，支持并行计算）
        在计算权重后，使用 LLM uncertainty gate 进行风险控制
        ...
        if not self.use_parallel or len(scores) <= 1:
            ...
            for stock_code, score in scores.items():
                ...
                # 应用 LLM uncertainty gate
                if apply_llm_gate and weight > 0:
                    logger.info(
                        f"[{self.name}] 开始 LLM Gate 评估: {stock_code}, "
                    ...
                    if not gate_result.get("should_execute", True):
                        # LLM 建议不执行交易，设置 weight=0
                        logger.info(
                            f"[{self.name}] LLM Gate 拒绝交易: {stock_code}, "
                            f"score={score:.4f}, weight={weight:.2%}, "
                        ...
                        logger.info(
                            f"[{self.name}] LLM Gate 通过交易: {stock_code}, "
                            f"score={score:.4f}, weight={weight:.2%}, "
                            f"reasoning={gate_result.get('reasoning', 'N/A')[:100]}"
                        )
                        agent.gate_passed_count += 1
                elif apply_llm_gate and weight == 0:
                    logger.debug(
                        f"[{self.name}] 跳过 LLM Gate 评估: {stock_code}, "
                        f"weight=0 (无需评估)"
                    )
