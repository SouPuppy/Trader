...
| CSV 原始列名 | `raw_data` 字段 | 类型 | 说明 | 约束 | `news_data` 字段 | 类型 | 说明 | 约束 |
|-------------|----------------|------|------|------|------------------|------|------|------|
| (空列) | `id` | INTEGER | 行情记录主键，自增 ID | PRIMARY KEY, AUTOINCREMENT | `id` | INTEGER | 新闻记录主键，自增 ID | PRIMARY KEY, AUTOINCREMENT |
| (空列，实际为日期) | `datetime` | TEXT | 行情日期（从 CSV 第一列读取） | |  |  |  |  |
|  |  |  |  |  | `uuid` | TEXT | 分析后新闻的唯一标识 | UNIQUE, NOT NULL |
|  |  |  |  |  | `paraphrase` | TEXT | 新闻主题 / 摘要（模型生成） | |
|  |  |  |  |  | `sentiment` | INTEGER | 新闻情绪方向，范围 [-10, 10] | CHECK(sentiment >= -10 AND sentiment <= 10) |
|  |  |  |  |  | `impact` | INTEGER | 新闻影响强度，范围 [0, 10] | CHECK(impact >= 0 AND impact <= 10) |
|  |  |  |  |  | `publish_date` | TEXT | 新闻发布日期（UTC 时间，从 news JSON 中提取的 publish_time 并转换为 UTC） | |
...
- `news` (CSV: `新闻`): JSON 格式字符串，包含以下字段：
  ...
  注意：`publish_time` 字段会被提取并转换为 UTC 时间存储在 `news_data` 表的 `publish_date` 字段中（仅在 analyzed 之后）。
...
**news_data 表字段说明**
- `publish_date` (系统字段): 新闻发布日期（UTC 时间），从 `news` JSON 中的 `publish_time` 字段提取并转换为 UTC
  - 类型: TEXT
  - 格式: "2023-01-02 23:15:25 UTC"（UTC 时区）
  - 用途: 方便按日期查询和筛选新闻，统一使用 UTC 时区避免时区混乱
  - 注意: 
    - 此字段仅在新闻被分析后才会填充到 `news_data` 表中
    - 如果 `news` JSON 解析失败或没有 `publish_time` 字段，此字段为 NULL
    - 如果原始时间没有时区信息，假设为 UTC
    - 如果原始时间有时区信息，会自动转换为 UTC
