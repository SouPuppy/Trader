"""
实验 6.3：层级式多资产交易系统（带 RAG 反思层 + 2-shot Few-Shot Learning）

基于 6.1 实验，使用多资产组合，RAG 反思层使用 2-shot few-shot learning（few_shot_count=2）

Layer A: Signal Layer（信号层）
Layer B: Allocation & RiskControl Layer（分配与风控层）
Layer C: RAG 反思层（参数θ每周调整，使用 RAG 系统 + 2-shot few-shot learning）

与 6.1 的关系：
- 6.1 是基准版本，不使用 few-shot learning（few_shot_count=0）
- 6.2 使用 1-shot few-shot learning
- 6.3 使用 2-shot few-shot learning，用于对比 few-shot 的效果
- 6.4 使用 3-shot few-shot learning
...
def hierarchical_multiasset_strategy_with_rag_reflection(
    ...
    """
    层级式多资产交易策略（带 RAG 反思层 + 2-shot Few-Shot Learning）
    ...
    Args:
        ...
        few_shot_count: Few-shot examples 数量（默认2，2-shot few-shot learning）
    """
    ...
    for line in log_section("层级式多资产交易系统（带 RAG 反思层 + 2-shot Few-Shot Learning）"):
        logger.info(line)
    ...
    report_file = engine.generate_parametrized_report(
        ...
        strategy_name="层级式多资产交易系统（带 RAG 反思层 + 2-shot Few-Shot Learning）"
    )
...
if __name__ == "__main__":
    # 执行层级式多资产交易策略（带 RAG 反思层 + 2-shot Few-Shot Learning）
    hierarchical_multiasset_strategy_with_rag_reflection(
        ...
        few_shot_count=2  # 2-shot few-shot learning
    )
