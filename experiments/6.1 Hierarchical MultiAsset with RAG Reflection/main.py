"""
实验 6.1：层级式多资产交易系统（带 RAG 反思层 - 基准版本）

基于 4.3 实验，使用多资产组合，RAG 反思层不使用 few-shot learning（few_shot_count=0）

Layer A: Signal Layer（信号层）
Layer B: Allocation & RiskControl Layer（分配与风控层）
Layer C: RAG 反思层（参数θ每周调整，使用 RAG 系统，基准版本不使用 few-shot）

与 4.3 的关系：
- 6.1 是 4.3 的多资产版本，作为 few-shot learning 实验的基准对比
- 6.2/6.3/6.4 分别使用 1/2/3-shot few-shot learning，用于对比 few-shot 的效果
...
def hierarchical_multiasset_strategy_with_rag_reflection(
    ...
    Args:
        ...
        few_shot_count: Few-shot examples 数量（默认0，基准版本不使用 few-shot）
    """
    ...
    for line in log_section("层级式多资产交易系统（带 RAG 反思层 - 基准版本）"):
        logger.info(line)
    ...
    report_file = engine.generate_parametrized_report(
        ...
        strategy_name="层级式多资产交易系统（带 RAG 反思层 - 基准版本）"
    )
...
if __name__ == "__main__":
    ...
    hierarchical_multiasset_strategy_with_rag_reflection(
        ...
        few_shot_count=0  # 基准版本：不使用 few-shot learning
    )
