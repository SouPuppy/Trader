"""
Logistic Regression 逻辑回归策略回测示例（带 LLM 风险控制）
使用 LogisticAgentWithLLMGate 实现基于逻辑回归的预测策略，增加投资比例并通过 LLM Gate 进行风险控制

参数设置：
- max_position_weight=0.5 (50%) - 单个股票最大仓位（增加投资力度，方便风险控制）
- max_total_weight=1.0 (100%) - 总仓位上限
- LLM Gate: 在执行交易前，使用 LLM 评估是否应该执行
"""
import sys
...
def logistic_strategy_with_llm_gate(
    ...
    """
    逻辑回归策略回测（使用 LogisticAgentWithLLMGate，增加投资比例 + LLM Gate 风险控制）
    ...
    Args:
        ...
        end_date: 结束日期
    """
    for line in log_section("逻辑回归策略回测（增加投资比例 + LLM Gate 风险控制）"):
        logger.info(line)
    ...
    # 创建带 LLM Gate 的逻辑回归 Agent
    agent = LogisticAgentWithLLMGate(
        name="Logistic_LLMGate_Strategy",
    ...
    def on_trading_day(eng: BacktestEngine, date: str):
        ...
        # 使用 LLM Gate 评估是否应该执行交易
        logger.info(f"[{agent.name}] 开始 LLM Gate 评估: {stock_code} on {date}, score={score:.4f}, weight={weight:.2%}")
        ...
        if not should_execute:
            ...
            logger.info(
                f"[{agent.name}] LLM Gate 拒绝执行交易: {stock_code} on {date}, "
        ...
        logger.info(
            f"[{agent.name}] LLM Gate 通过，执行交易: {stock_code} on {date}, "
            f"score={score:.4f}, weight={weight:.2%}, "
            f"原因: {reasoning[:100]}..."
        )
